...
export function InfoPanel({
  ...
  const isPlanet = !isDeepSpace;

  const deepSpace = isDeepSpace ? (selectedItem as DeepSpaceObject) : null;
  const planet = isPlanet ? (selectedItem as CelestialBody) : null;

  return (
    <aside className="fixed top-0 right-0 bottom-0 z-50 pointer-events-none flex justify-end">
      {/* Sleek Right-Side Glassmorphic Drawer without Fullscreen Backdrop Blur */}
      <div
        className="w-88 sm:w-96 h-full bg-[#0c061a]/95 backdrop-blur-2xl border-l border-purple-900/50 shadow-[-15px_0_50px_rgba(0,0,0,0.85)] flex flex-col pointer-events-auto text-slate-100 select-none animate-in slide-in-from-right duration-250 overflow-hidden"
        onClick={(e) => e.stopPropagation()}
      >
        {/* 1. Header Card */}
        <div className="p-5 pb-3 border-b border-purple-900/40 bg-gradient-to-b from-purple-950/60 to-transparent space-y-2 relative shrink-0">
              ...
              {deepSpace ? (
                <>
                  {[
                    ...
                    { label: 'Energy / Temp', val: deepSpace.stats.temperatureOrEnergy },
                ...
                planet && (
                  <>
                    {[
                      ...
                      { label: 'Mean Temp', val: planet.stats.temperature },
                    ].map((item) => (
                      <div
                        key={item.label}
                  ...
                  {planet.exploration && planet.exploration.length > 0 && (
                    <div className="bg-purple-950/30 border border-purple-900/40 rounded-2xl p-4 space-y-2">
                      <div className="text-[11px] font-mono font-bold text-purple-300 flex items-center gap-1.5">
                        <Rocket className="w-3.5 h-3.5 text-purple-400" /> Exploration Missions
                      </div>
          ...
          {activeTab === 'facts' && (
            <div className="space-y-2.5">
              {(planet ? planet.funFacts : deepSpace?.facts || []).map((fact, idx) => (
                <div
                  key={idx}
                  className="flex items-start gap-3 p-3 rounded-xl bg-purple-950/30 border border-purple-900/40"
                >
        ...
        {planet && planet.id !== 'earth' && onCompareWithEarth && (
          <div className="p-4 border-t border-purple-900/40 bg-purple-950/40 flex items-center justify-between gap-3 shrink-0">
            <button
              onClick={() => onCompareWithEarth(planet.id)}
              className="flex-1 py-2.5 px-4 rounded-xl bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-500 hover:to-indigo-500 text-white font-bold flex items-center justify-center gap-2 transition-all shadow-[0_0_20px_rgba(168,85,247,0.4)] cursor-pointer text-xs"
            >
